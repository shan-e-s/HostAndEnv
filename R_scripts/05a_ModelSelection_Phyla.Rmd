...
all.fir.2 <- glmer(binomial.response ~  ageBinned * habitat * layDateFirst + ageBinned * broodSizeWhenSampled + DistanceToEdge +  (1|site/nest/bird.ID) + (1|SequencePlate), family= binomial, data = firmicutes.df) # fits well
...
```{r}
simulationOutput <- simulateResiduals(fittedModel = all.fir, n = 250)
plot(simulationOutput)
```
