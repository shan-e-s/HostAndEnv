...
p1 <- plot_grid(distance.shannon.all+theme_classic2(), distance.chao.all+theme_classic2(), nrow = 1,rel_widths = c(1,1,0.4)) #p4.legend
p1
```
```{r}
#p4.legend <- get_legend(broodSizeXage.proteo.all) # from script 05c_

p1.age <- plot_grid(distance.shannon.all+theme_classic2(), distance.chao.age+theme_classic(), nrow = 1,rel_widths = c(1,1.3,0.4)) #p4.legend
p1.age
```


```{r}
ageXhabitat.chao.all+theme_classic()
```
