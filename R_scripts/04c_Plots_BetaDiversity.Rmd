...
p <- p + geom_point(show.legend = F, aes(shape = ageBinned)) + xlab(PC1.label) + ylab(PC2.label) #, show.legend = F
p

p <- p + stat_ellipse(geom = "polygon", type="t", alpha=0, 
                      aes(group = ageBinned, colour = ageBinned)) #colour = c("red", "blue", "orange"),
p
