...
* paper in directory/Papers/Repeatability: Repeatability for Gaussian and non‐Gaussian data a practical guide for biologists by Nakagawa
...
## Site only
```{r}
rpt.shannon.site.only <- rpt(log(shannon) ~ (1|site), grname = "site", data = nestling.metadata, datatype = "Gaussian", nboot = 100, nperm = 10000, parallel = T)
...
```{r}
rpt.shannon.birdID.nest.site.fixed <- rpt(log(shannon) ~ ageBinned*habitat + (1|bird.ID) + (1|nest) + (1|site), grname = c("bird.ID", "nest", "site", "Fixed"), data = nestling.metadata, datatype = "Gaussian", nboot = 1000, nperm = 1000, parallel = T)
