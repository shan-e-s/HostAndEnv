...
```
```{r}
#nstlng.adult.df <- droplevels(nstlng.adult.df)

ggplot(nstlng.adult.df, aes(log(both.either.fir.RA), nst.fir.abu/nst.NumberOfReads, fill = both.either.char)) +
  geom_smooth(method = "lm") +
  geom_point() +
  facet_grid(~both.either.char)

#min(nstlng.adult.df$both.either.fir.RA)
```

```{r}

ggplot(nstlng.adult.df, aes(both.either.shannon, nst.shannon, fill = both.either.char)) +
  geom_smooth(method = "lm")+
  geom_point(aes(color = both.either.char))
